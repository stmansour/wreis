...
// SetNoAuthFlag enable/disable authentication in Wdb
// ON production we should not allow this flag to set
func SetNoAuthFlag(noauth bool) {
	if Wdb.Config.Env != extres.APPENVPROD { // NOT only applicable for PROD Environment
		Wdb.noAuth = noauth
	}
}
...
func SessionCleanup() {
	for {
		...
		case <-time.After(SessionCleanupTime * time.Minute):
			...
			// util.Console("SessionCleanup completed. %d removed. Current Session list size = %d\n", n, len(sessions))
		}
	}
}
...
//  session - pointer to the new session
//-----------------------------------------------------------------------------
func CreateSession(ctx context.Context, a *util.AIRAuthenticateResponse) (*Session, error) {
	expiration := time.Time(a.Expire)

	// use a network call for this...
	// //----------------------------------------------
	// // TODO: lookup username in address book data
	// //----------------------------------------------
	// util.Console("Calling GetDirectoryPerson with UID = %d\n", a.UID)
	// var c util.DirectoryPerson
	// err := Wdb.PBsql.GetDirectoryPerson.QueryRow(a.UID).Scan(&c.UID, &c.UserName, &c.LastName, &c.MiddleName, &c.FirstName, &c.PreferredName, &c.PreferredName, &c.OfficePhone, &c.CellPhone)
	// SkipSQLNoRowsError(&err)
	// if err != nil {
	// 	var bad Session
	// 	util.Console("*** ERROR *** GetDirectoryPerson - %s\n", err.Error())
	// 	return &bad, err
	// }
	// util.Console("Successfully read info from directory for UID = %d\n", c.UID)

	RoleID := int64(0) // we haven't yet implemented Role
	s := SessionNew(a.Token, "username-unknown", "Name Unknown", a.UID, a.ImageURL, RoleID, &expiration)
	return s, nil
}
...
func ValidateSessionCookie(r *http.Request) (ValidateCookie, error) {
	...
	//-----------------------------------------------------------------------
	url := Wdb.Config.AuthNHost + "v1/validatecookie"
...
func GetSession(ctx context.Context, w http.ResponseWriter, r *http.Request) (*Session, error) {
	funcname := "GetSession"
	var b util.AIRAuthenticateResponse
	var ok bool
...
func SessionDelete(s *Session, w http.ResponseWriter, r *http.Request) {
	if nil == s {
		util.Console("SessionDelete: supplied session is nil\n")
		return
	}
