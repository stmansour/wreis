		...
		for i := 0; i < len(d.wsSearchReq.Search); i++ {
			...
			for j := 0; j < val.NumField(); j++ {
				...
				case "between":
					qw = gridHandleField(qw, d.wsSearchReq.SearchLogic, d.wsSearchReq.Search[i].Field, d.wsSearchReq.Search[i].Value, " %s like '%%%s%%'", &count)
				default:
					// util.Console("Unhandled search operator: %s\n", d.wsSearchReq.Search[i].Operator)
				}
			}
		}
		if len(qw) > 0 {
...
func GetRowCount(table, where string) (int64, error) {
	...
	de := db.Wdb.DB.QueryRow(s).Scan(&count)
	if de != nil {
...
// HandleBlankSearchField updates the search structure of d such that for every
// search item in the array where the field name is blank, it replaces that
...
func HandleBlankSearchField(d *ServiceData, dsf []string) {
	...
	for _, gsrch := range d.wsSearchReq.Search {
		if len(gsrch.Field) == 0 && len(gsrch.Value) > 0 {
			for j := 0; j < len(dsf); j++ {
				g := GenSearch{
					Field:    dsf[j],
					Type:     gsrch.Type,
					Value:    gsrch.Value,
					Operator: gsrch.Operator,
				}
				ga = append(ga, g)
			}
		}
	}
	d.wsSearchReq.Search = ga
}
...
func GetSearchAndSortSQL(d *ServiceData, fieldMap map[string][]string) (string, string) {
	...
	// util.Console("get search clause\n")
	for _, gsrch := range d.wsSearchReq.Search {
		...
		if _, ok := fieldMap[gsrch.Field]; !ok {
			// util.Console("Coult not find in fieldMap:  %s\n", gsrch.Field)
			continue
		}
		reqSearchClauseMap[gsrch] = fieldMap[gsrch.Field]
	}
	// util.Console("get where clause\n")
	// util.Console("    reqSearchClauseMap = %#v\n", reqSearchClauseMap)
	// util.Console("    d.wsSearchReq.SearchLogic = %#v\n", d.wsSearchReq.SearchLogic)
